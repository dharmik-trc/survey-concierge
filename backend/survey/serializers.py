...
from .count_utils import count_all_responses, count_completed_responses, count_partial_responses
from .models import PartialSurveyResponse, Question, QuestionResponse, Survey, SurveyResponse
        ...
        if question_type == "email":
            ...
            if question.options and answer:
                ...
                def is_valid_choice(choice):
                    ...
                    if isinstance(choice, dict) and "answer" in choice:
                        ...
                        if isinstance(actual_answer, list):
                            ...
                            return is_valid_choice(
                                actual_answer
                            )  # Recursively validate the actual answer
                    ...
                    if (
                        question.has_none_option
                        and question.none_option_text
                        and str(choice).lower() == question.none_option_text.lower()
                    ):
                        return True
            ...
            if not isinstance(answer, dict):
                raise serializers.ValidationError(
                    "Form fields answer must be an object with subfields."
                )
                    ...
                    if subfield in question.subfields and value is not None and value != "":
                        ...
                        if validation_type in ["positive_number", "negative_number", "all_numbers"]:
                            if not self._is_valid_number(value):
                                raise serializers.ValidationError(
                                    f"Subfield '{subfield}' must be a number."
                                )
                            ...
                            if not re.match(email_regex, str(value)):
                                raise serializers.ValidationError(
                                    f"Subfield '{subfield}' must be a valid email address."
                                )
            ...
            if not isinstance(answer, dict):
                raise serializers.ValidationError(
                    "Cross matrix answer must be an object mapping rows to columns."
                )
                ...
                for row, value in answer.items():
                    if row in question.rows and value is not None and value != "":
                        if value not in question.columns:
                            raise serializers.ValidationError(
                                f"Invalid column for row '{row}': {value}"
                            )
            ...
            if not isinstance(answer, dict):
                raise serializers.ValidationError(
                    "Cross matrix checkbox answer must be an object mapping rows to arrays of columns."
                )
            if question.rows and question.columns and answer:
                # Only validate rows that are actually provided
                for row, value in answer.items():
                    if row in question.rows and value is not None:
                        if not isinstance(value, list):
                            raise serializers.ValidationError(
                                f"Row '{row}' must be an array of selected columns."
                            )
                        for col in value:
                            if col not in question.columns:
                                raise serializers.ValidationError(
                                    f"Invalid column for row '{row}': {col}"
                                )
            ...
            if question.is_required and question.rows and answer:
                for row in question.rows:
                    if row in answer and (not answer[row] or len(answer[row]) == 0):
                        raise serializers.ValidationError(
                            f"At least one column must be selected for row '{row}'"
                        )
        ...
        if question.is_required:
            if question_type in ["multiple_choices", "radio", "dropdown", "yes_no", "fields"]:
                ...
                if not answer or not isinstance(answer, dict) or not answer:
                    ...
                    if any((v == "" or v is None) for v in answer.values() if v is not None):
                        raise serializers.ValidationError(
                            "All subfields must be filled for required questions."
                        )
            else:
    ...
    def _validate_character_limits(self, question, answer, answer_type):
        ...
        if question_type in ["text", "paragraph"]:
            ...
            if isinstance(answer, str) and len(answer) > MAX_TEXT_LENGTH:
                raise serializers.ValidationError(
                    f"Text response cannot exceed {MAX_TEXT_LENGTH} characters"
                )
            ...
            if isinstance(answer, str) and len(answer) > MAX_NUMBER_LENGTH:
                raise serializers.ValidationError(
                    f"Number cannot exceed {MAX_NUMBER_LENGTH} characters"
                )
...
class SurveyResponseSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        answered_question_ids = [
            qr.get("question").id for qr in question_responses if qr.get("question")
        ]

        for question in survey_questions:
            if question.is_required and question.id not in answered_question_ids:
                raise serializers.ValidationError(
                    f"Required question '{question.question_text}' is not answered"
                )
    ...
    def create(self, validated_data):
        ...
        for response_data in question_responses_data:
            QuestionResponse.objects.create(survey_response=survey_response, **response_data)
