...
from ..models import PartialSurveyResponse, QuestionResponse, SurveyResponse
...
def collect_session_data(survey):
    ...
    partial_responses = (
        PartialSurveyResponse.objects.filter(
            survey=survey, is_completed=False  # Only get incomplete partial responses
        )
    ...
    for response in partial_responses:
        ...
        if session_id not in sessions:
            sessions[session_id] = {
                ...
                "session_id": str(
                    session_id
                ),  # Store session_id for Respondent ID (partial responses)
                "questions": {},
            }
...
def merge_completed_responses(survey, sessions, completed_session_ids):
    ...
    survey_response_ids = SurveyResponse.objects.filter(survey=survey).values_list("id", flat=True)
    ...
    for response_id, data in by_response.items():
        ...
        sessions[session_id] = {
            ...
            "survey_response_id": str(response_id).split("-")[
                -1
            ],  # Store the UUID for Respondent ID
            "questions": {},
        }
...
def analyze_question_subfields(sessions, questions):
    ...
    for question in questions:
        if question.secondary_type == "multiple_choices" and question.options:
            ...
            if question.has_none_option:
                none_text = (
                    question.none_option_text if question.none_option_text else "None of the above"
                )
    ...
    for question in questions:
        ...
        is_grid = primary_type == "grid" or question_type in [
            "cross_matrix",
            "grid_radio",
            "cross_matrix_checkbox",
            "grid_multi",
        ]
        if is_grid:
    ...
    question_subfields = {
        qid: sorted(list(subfields)) for qid, subfields in question_subfields.items()
    }
...
def collect_completed_responses_only(survey):
    ...
    for qr in question_responses:
        ...
        if response_id not in sessions:
            sessions[response_id] = {"questions": {}}
