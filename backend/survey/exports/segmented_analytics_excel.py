...
def create_segmented_analytics_excel(
    ...
    for question in questions:
        ...
        has_analytics = any(
            question_id in segment_analytics.get(seg, {})
            for seg in segments
        )
        ...
        for seg_name in segments:
            ...
            ws.cell(row=row_num, column=col + 1).alignment = Alignment(horizontal='center', vertical='center')
...
def _write_segmented_form_fields_numeric_analytics(
    ...
    for subfield_name in sorted(all_subfields):
        # Subfield header
        ws.merge_cells(start_row=row_num, start_column=1, end_row=row_num, end_column=1 + (len(segments) * cols_per_segment))
        cell = ws.cell(row=row_num, column=1)
            ...
            for seg in segments:
                ...
                if q_analytics and q_analytics.get('type') == 'form_fields_numeric':
                    ...
                    if stat_value is not None and stat_value != 'N/A':
                        value = round(stat_value, 2) if isinstance(stat_value, (int, float)) else stat_value
