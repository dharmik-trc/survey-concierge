...
def calculate_analytics(
    ...
    for question in questions:
            ...
            for session_id, session_data in sessions.items():
                ...
                if isinstance(answer, dict) and "comment" in answer:
                    ...
                    if comment_text and str(comment_text).strip():
                        ...
                        comments.append(
                            {
                                "response_id": str(metadata.get("response_id", session_id))[
                                    -12:
                                ],  # Last 12 chars
        ...
        if primary_type == "grid":
            ...
            try:
                for candidate in ["Total", "total", "Sum", "sum"]:
                    if (
                        hasattr(question, "subfields")
                        and question.subfields
                        and candidate in question.subfields
                    ):
        ...
        elif primary_type == "open_text" and question_type in [
            "number",
            "positive_number",
            "negative_number",
        ]:
            ...
            if (
                numeric_result
                and isinstance(numeric_result, dict)
                and numeric_result.get("type") == "numeric"
            ):
                ...
                analytics[question_id] = {
                    ...
                    "skipped_count": skipped_count,
                    "min": (
                        numeric_result.get("min", "N/A")
                        if numeric_result and numeric_result.get("count", 0) > 0
                        else "N/A"
                    ),
                    "q1": (
                        numeric_result.get("q1", "N/A")
                        if numeric_result and numeric_result.get("count", 0) > 0
                        else "N/A"
                    ),
                    "median": (
                        numeric_result.get("median", "N/A")
                        if numeric_result and numeric_result.get("count", 0) > 0
                        else "N/A"
                    ),
                    "q3": (
                        numeric_result.get("q3", "N/A")
                        if numeric_result and numeric_result.get("count", 0) > 0
                        else "N/A"
                    ),
                    "max": (
                        numeric_result.get("max", "N/A")
                        if numeric_result and numeric_result.get("count", 0) > 0
                        else "N/A"
                    ),
                    "average": (
                        numeric_result.get("average", "N/A")
                        if numeric_result and numeric_result.get("count", 0) > 0
                        else "N/A"
                    ),
                    "sum": (
                        numeric_result.get("sum", "N/A")
                        if numeric_result and numeric_result.get("count", 0) > 0
                        else "N/A"
                    ),
                }
...
def calculate_choice_analytics(
    question, sessions: Dict, answered_count: int, skipped_count: int, comments: List
) -> Dict[str, Any]:
    """
    ...
    for session_id, session_data in sessions.items():
        ...
        if not selected_options or (
            isinstance(selected_options, list) and len(selected_options) == 0
        ):
            continue
    ...
    for option in ordered_options:
        ...
        results.append({"option": option, "count": count, "percentage": round(percentage, 2)})
...
def calculate_grid_analytics(
    ...
    for row in rows:
        ...
        for col in columns:
            ...
            column_stats.append({"column": col, "count": count, "percentage": round(percentage, 2)})

        row_results[row] = {"total_responses": total_row_responses, "columns": column_stats}
...
def calculate_numeric_analytics(
    question, sessions: Dict, answered_count: int, skipped_count: int
) -> Dict[str, Any]:
    """
...
def calculate_form_fields_numeric_analytics(
    question, sessions: Dict, answered_count: int, skipped_count: int, numeric_subfields: List[str]
) -> Dict[str, Any]:
    """
